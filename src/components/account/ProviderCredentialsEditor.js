...
const ProviderCredentialsEditor = (props) => {
  ...
  const onDisabledSaveButton = (u) => {
    if (user && loadUser && user.firstName === loadUser.firstName && user.lastName === loadUser.lastName && user.email === loadUser.email && user.role === loadUser.role) {return true;}
    if (!user || !user.firstName || user.firstName === '') {return true;}
    if (!user.lastName || user.lastName === '') {return true;}
  ...
  return (
    <S.CredentialEditWrap>
      <S.CredentialEditColumn>
        ...
        <S.CredentialEditColumnInput type="text" name="firstName" value={user && user.firstName ? user.firstName : ''} onChange={onFormChanged} disabled={readOnly}/>
      </S.CredentialEditColumn>
      <S.CredentialEditColumn>
        <div >{t('account.credentials.lastName')}</div>
        <S.CredentialEditColumnInput type="text" name="lastName" value={user && user.lastName ? user.lastName : ''} onChange={onFormChanged} disabled={readOnly}/>
      </S.CredentialEditColumn>
      <S.CredentialEditRow>
        {t('account.credentials.email')}
      </S.CredentialEditRow>
      <S.CredentialEditRow>
        <S.CredentialEditRowInput type="text" name="email" value={user && user.email ? user.email : ''} onChange={onFormChanged} disabled={readOnly}/>
      </S.CredentialEditRow>
      <S.CredentialEditRow>
        {t('account.credentials.role')}
      </S.CredentialEditRow>
      <S.CredentialEditRow>
        <S.CredentialEditRowSelect name="role" onChange={onFormChanged} disabled={readOnly} value={selectValue(user && user.role ? user.role : undefined)}>
